...
- **Never commit**: CSV exports, `.env`, tokens, customer/order data, or private vault material. `.gitignore` enforces some of this; you enforce the rest. Before any push, two passes: scan **added** diff lines for tokens and gmail addresses (added only, since deleting a secret is not a leak), and scan changed **paths** (`git diff origin/main --name-only`) for the `.env.*` family and `.csv`. Three narrowings, all to stop the guard crying wolf: `.csv`/`.env` match on paths only (page copy may legitimately name an export file); the content pass skips `CLAUDE.md` alone, which spells the patterns out and matches itself, via `:(top,exclude)` so it still works from a subdirectory; and `opensourcewarren@gmail.com` is exempt, being the deliberately public contact address (formerly published on the retired `/archive-services` page). Every other gmail address still trips, near-misses and odd capitalisation included. Exempt that one address only, and never widen it back to a bare `@gmail`. Exact commands in CLAUDE.md rule 1.
